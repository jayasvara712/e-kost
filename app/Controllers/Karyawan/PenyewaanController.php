...
class PenyewaanController extends BaseController
{
    ...
    public function bayar_cod($id)
    {
        ...
        if ($cekData) {
            ...
            $data2 = [
                'last_transaction_status'    => 'settlement',
                'last_transaction_time'      => date("Y-m-d h:i:s"),
                'last_payment'          => $cekData->payment,
            ];
            $this->modelPenyewaanDetail->update($id, $data1);
            $this->modelPenyewaan->update($cekData->id_penyewaan, $data2);

            $json = [
                'success' => 'Kamar sudah dibayar!'
            ];
            echo json_encode($json);
        } else {
            $json = [
                'error' => 'Bayar penyewaan gagal!'
            ];
            echo json_encode($json);
        }
    }
